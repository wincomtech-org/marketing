...
// 赋值给模板-导航栏
$smarty->assign('nav_list', $dou->get_nav_mobile('mobile', 0, 'user', 0));
...
if ($rec == 'default') {
    ...
    $smarty->assign('head', $dou->head($_LANG['user']));
...
elseif ($rec == 'register') {
    ...
    $smarty->assign('head', $dou->head($_LANG['user_register']));
...
elseif ($rec == 'login') {
    ...
    $smarty->assign('head', $dou->head($_LANG['user_login']));
...
elseif ($rec == 'password_reset') {
    ...
    $smarty->assign('head', $dou->head($_LANG['user_password_reset']));
...
elseif ($rec == 'password_reset_post') {
    ...
    if ($action == 'default') {
        ...
        if ($dou->send_mail($_POST['email'], $_LANG['user_password_reset_title'], $body)) {
            $dou->dou_msg($_LANG['user_password_mail_success'] . $user['email'], M_URL, 30);
        } else {
        ...
        if ($dou_user->check_password_reset($user_id, $code)) {
            ...
            $dou->dou_msg($_LANG['user_password_reset_fail'], M_URL, 30);
        }
    }
}
...
elseif ($rec == 'edit') {
    ...
    $smarty->assign('head', $dou->head($_LANG['user_edit']));
    $smarty->assign('user_info', $user_info);
...
elseif ($rec == 'password') {
    ...
    $smarty->assign('head', $dou->head($_LANG['user_password_edit']));
...
elseif ($rec == 'order_list') {
    ...
    while ($row = $dou->fetch_array($query)) {
        ...
        $shipping_fee_format = $dou->price_format($row['shipping_fee']);
    ...
    $smarty->assign('head', $dou->head($_LANG['order_my']));
...
elseif ($rec == 'order') {
    ...
    $smarty->assign('head', $dou->head($_LANG['order_view']));
