...
// 如果为末级分类则显示当前同级分类
if ($dou->dou_child_id('video_category', $cat_id)) {
    $parent_id = $cat_id;
} else {
...
// 赋值给模板-导航栏
$smarty->assign('nav_list', $dou->get_nav_mobile('mobile', '0', 'video_category', $cat_id, $cate_info['parent_id']));

// 赋值给模板-数据
$smarty->assign('head', $dou->head($_LANG['video_category']));
$smarty->assign('ur_here', $dou->ur_here('video_category', $cat_id));
$smarty->assign('cate_info', $cate_info);
$smarty->assign('video_category', $dou->get_category('video_category', $parent_id, $cat_id));
