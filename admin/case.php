...
if ($rec == 'default') {
    ...
    $sql = sprintf("SELECT %s,b.cat_name from %s a left join %s b on a.cat_id=b.cat_id %s ORDER BY %s %s", $fields,$dou->table('case'),$dou->table('case_category'),$where,'a.cat_id,a.add_time DESC',$limit);
    $query = $dou->query($sql);
