...
/* 获取详细信息 */
$article = $dou->fetchRow("SELECT * FROM " . $dou->table('article') . " WHERE id = '$id'");
...
// $article = $dou->fetch_array($query,MYSQL_ASSOC);
...
// 访问统计
$dou->query("update " . $dou->table('article') . " SET click = '". $article['click']+1 ."' WHERE id = '$id'");
...
$_CFG['deftpl']['article'] = 'report/article_info.html';
$thistpl = $article['template']?$article['template']:$_CFG['deftpl']['article'];
